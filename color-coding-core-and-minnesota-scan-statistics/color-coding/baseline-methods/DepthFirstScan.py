...
def depth_first_subgraph_detection(graph, att,npss, radius = 7, anomaly_ratio = 0.5, minutes = 30, alpha_max = 0.15):
    ...
    for alpha_i, alpha in enumerate([alpha_max]):
            ...
            if not subset_score or seed[0] not in subset_score[0] or seed_i == 0:
                ...
                while priqueue.len() > 0:
                    ...
                    if iter % 5000 == 0:
                        ...
                        if duration > minutes * 60 or iter > 10000:
...
def test_DepthFirst_waterNetwork():
    numOfThreads = 1
    resultNodesFileName = 'depthFirstScan_waterNetwork_resultNodes.txt'
    resultFileTableName = 'depthFirstScan_waterNetwork_resultTable.txt'
    dataRootFolder = '../data/water-data/WaterData/source_12500'
    radius = 10
    anomalyRate = 0.5
    minutes = 200.0
    alphaMax = 0.15
    files = os.listdir(dataRootFolder)
    for eachFile in files:
        print 'start to process : ',eachFile
        fileName = eachFile.split('.txt')[0]
        graph, att,true_subGraph = read_APDM_data_waterNetwork(os.path.join(dataRootFolder,eachFile))
        start_time = time.time()
        subset_score = depth_first_subgraph_detection(graph, att,'BJ',radius,anomalyRate,minutes,alphaMax)
        pre,rec,fmeasure = prec_recall_fmeasure(subset_score[0],true_subGraph)
        runningTime = time.time() - start_time
        print 'finish processing : ',eachFile
        ...
        file = open(resultNodesFileName,'a')
        file.write(fileName)
...
def run():
    ...
    subset_score = depth_first_subgraph_detection(graph, att, 'BJ')
