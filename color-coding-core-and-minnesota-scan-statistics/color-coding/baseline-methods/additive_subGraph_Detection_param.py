...
def f_score(subset, att, alpha = 0.05):
    weight = sum([att[nid] for nid in subset])
    return weight / sqrt(len(subset))
...
def refine_graph(graph1, att1, alpha):
    ...
    for comp in comps:
        ...
        graph[n] = neis
        #att.append(len(comp))
        att.append(sum(att1[i] for i in comp))
        compdict[n] = comp
        n += 1
...
def additive_graphscan(graph, att, ncores = 7, iterations_bound = 6, minutes = 5, pri = False):
    ori_graph = graph
    ori_att = att
    alpha_max = 0.15
...
def additive_graphscan_proc(graph, att, compdict, alpha, iterations_bound = 10, ncores = 7, minutes = 2, pri = False):
    flag = True
    sfset = None
    sfscore = None
    mset = []
...
def run():
    ...
    start_time = time.time()
    graph, att,true_subgraph = read_apdm(fname)
    subset_score = additive_graphscan(graph, att)
    timeV = time.time() - start_time

    with open(outf, 'a') as f:
        f.write(fname + "," + str(subset_score[1]) + "," + str(timeV) + "\n")
        f.write(str(subset_score[0]) + "\n")
   

def test_water_pollution_data():
    folderName = 'WaterData'
    folderName = 'waterData-noise-0'
    folderName = '/Users/jcadena/Dropbox/color-coding/water-dataset/WaterData'
    subFolderNames = os.listdir(folderName)
    subFolderNames = [fileName for fileName in subFolderNames if os.path.isdir(folderName+'/'+fileName) and 'without_noise' in fileName]

    for eachSubFolder in subFolderNames:
        print eachSubFolder
        for eachFileData in sorted(os.listdir(os.path.join(folderName,eachSubFolder))):
...
#            print att
            ...
            print eachFileData
            print prec,recall
            acc = len(set(subset_score[0]).intersection(set(true_subgraph)))
            acc = float(acc) / len(set(subset_score[0]).union(set(true_subgraph)))
            timeV = time.time() - start_time
            with open(eachFileData.replace('.txt', '.csv'), 'w') as outf:
                for i in subset_score[0]:
                    outf.write("%s\n" % i)
            #f.write(str(eachFileData.split("_")[5].split(".")[0]+" "+str(prec)+" "+str(recall)+" "+str(timeV)))
            f.write(str(eachFileData+" "+str(prec)+" "+str(recall)+" "+str(fscore)+" "+str(acc)+" "+str(subset_score[1])+" "+str(timeV)) + "\n")
            print("--- %s seconds ---" % (time.time() - start_time))
            f.close()
...
def read_apdm(path):
    ...
    for idx in range(n, len(lines)):
        line = lines[idx]
        if line.find('END') >= 0 or line.find('null') >= 0:
            break
        else:
            items = line.split(' ')
            true_subgraph.append(int(items[0]))
